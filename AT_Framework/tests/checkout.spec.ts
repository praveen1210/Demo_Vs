...
test.describe('Checkout Tests', () => {
  ...
  await expect(page).toHaveURL(/inventory.html/);
    ...
    const expectedSubtotal =Number(product1Price.replace('$', '')) +Number(product2Price.replace('$', ''));
    ...
    expect(await checkoutPage.getConfirmationMessage()).toBe(errorMessages.checkoutComplete);
  ...
  await expect(page).toHaveURL(/inventory.html/);
   ...
   expect(await checkoutPage.getErrorMessage()).toBe(errorMessages.checkoutFirstNameRequired);
  });
});
